...
class EventHandler{
    ...
    private calculateBonus(arrivalTime: string) {
        ...
        return arrivalTimeInMinutes < expectedArrivalTime - this.intervalTime ? this.bonusPoint : arrivalTimeInMinutes > expectedArrivalTime + this.bonusPoint ? -this.bonusPoint : 0;
    }

    async computeStudentRecord() {
        //XREAD records
        const events: any = await client.sendCommand(['XREAD', 'STREAMS', 'student', '0-0'])
        if(!events) return [];
    ...
    async computeAttendanceHandler(userId: string) {
        ...
        //XACK ALL Attendance
        const events: any = await client.sendCommand(['XREAD', 'STREAMS', 'attendance', '0-0'])
        if(events){
